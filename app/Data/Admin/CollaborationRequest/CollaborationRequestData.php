...
final class CollaborationRequestData extends Data
{
    ...
    public static function fromModel(CollaborationRequest $request): self
    {
        ...
        return new self($request->id, $request->full_name, $request->phone, $request->email, $request->department,
            $request->message, $request->status, $request->note,
            $request->assignee ? StaffData::from($request->assignee) : null,
            $attachment ? PrivateFileData::fromModel($attachment, 'attachment') : null,
            $request->created_at?->toISOString(), $request->updated_at?->toISOString());
    }
}
