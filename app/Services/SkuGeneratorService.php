...
class SkuGeneratorService
{
    ...
    public function generateBaseSku(ProductDeliveryOptionCreateData $data, Product $product): string
    {
        ...
        return strtoupper("{$productableCode}-{$termCode}-{$deliveryCode}");
    }
    ...
    private function generateProductableCode(object $productable): string
    {
        ...
        if (strlen($acronym) < 3) {
            $acronym = substr(str_replace('-', '', $slug), 0, 3);
        }

        return substr($acronym, 0, 5); // Max length of 5
    }
    ...
    private function generateTermCode(Term $term): string
    {
        ...
        $year = $term->academic_year ? substr($term->academic_year, 0, 4) : '0000';
