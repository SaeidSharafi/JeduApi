...
test('to array', function (): void {
    ...
    expect($seminar->toArray())
        ->toEqual([
            ...
            'curriculum_summary_text'  => $seminar->curriculum_summary_text,
            'outcomes_json'            => $seminar->outcomes_json,
...
test('with reviews', function (): void {
    ...
    expect($seminar->reviews)
        ->toHaveCount(1)
        ->and($seminar->reviews->first())
        ->toBeInstanceOf(App\Models\Review::class)
        ->and($seminar->reviews->first()->id)
        ->toEqual($review->id)
        ->and($seminar->reviews->first()->user->id)
        ->toEqual($user->id);
});
