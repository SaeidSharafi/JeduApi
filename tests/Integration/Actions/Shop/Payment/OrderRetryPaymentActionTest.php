...
describe('Retry Order Payment', function (): void {
    ...
    it('regression: retry with wallet succeeds when balance is sufficient', function (): void {
        $user = User::factory()->create();
        $user->wallet->update(['balance' => 500000]);
        ...
        $result = $action->handle($order, PaymentMethodEnum::WALLET);

        expect($result->payment)->not->toBeNull();
        expect($result->payment->status->value)->toBe('completed');
