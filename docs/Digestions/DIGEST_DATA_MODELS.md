...
- **Special Features:** UUID for external references, SKU auto-generation via `SkuGeneratorService` when not provided, and monotonic `composition_version` for Bundle PDO composition changes. Bundle PDOs use `fulfillment_type=composite`, `delivery_method=bundle`, empty details, `is_prepayment_available=false`, and `prepayment_amount=null`; create/update normalizes these structural invariants server-side. They atomically persist component allocations. Capacity tracking is backed by the persisted `enrolled_count` column (no more runtime `withCount`), and `discountPrice` evaluates active discount windows. **Capacity reservations:** `reserved_count` holds seats held by PENDING orders; committed seats = `enrolled_count + reserved_count`, and capacity validity checks compare `capacity > (enrolled_count + reserved_count)`. Reservations are managed by `ProductReservationService` (`reserve`/`consume`/`release`).
  Bundle PDOs also carry `bundle_review_required_at` and stable `bundle_review_reasons`. Material component changes archive containing Bundle PDOs and require the explicit staff review action before the review flag can be cleared. Bundle availability uses the least remaining finite component capacity; unlimited components do not cap the offer. Reservations for Bundle purchases are stored on component PDOs, never on the parent.
...
- **Special Features:** Its derived `status` (accessor, `BundlePurchaseStatusEnum`) is a deterministic function of the owning Order payment state and every component OrderItem status, its Enrollment lifecycle status, the Enrollment's aggregate provisioning health, and — once refunded — the per-component revocation state. Reachable values: `pending_payment`, `provisioning`, `active`, `partially_failed`, `failed`, `revocation_pending` (a refunded component's external provider access is not yet fully revoked), `refunded` (every required component revocation succeeded), and `cancelled` (unpaid order cancelled before payment). A Bundle Purchase owns no parent Order Item or Enrollment; internal components snapshot their base price, paid allocation, and Bundle discount in `pricing_metadata`. Component Enrollments are created at checkout (`awaiting_payment`) by `CreateBundlePurchaseAction`, then activated and provisioned through the ordinary payment/provider pipeline exactly like standalone Enrollments.

### Enrollment (`app/Models/Enrollment.php`)
- **Purpose:** Student access records linking customers to purchased delivery options
- **Key Fields:** `uuid`, `order_id`, `order_item_id`, `customer_id`, `product_delivery_option_id`, `enrollment_status` (`awaiting_payment | active | suspended | expired | cancelled`), `access_start_date`, `access_end_date`, `provisioning_data` (JSONB per-provider execution data), `provisioning_plan` (non-null versioned JSONB provider applicability/readiness snapshot), `provisioning_status` (aggregate provisioning health), `revocation_status` (`pending | failed | manual_action_required | revoked`, nullable — set by the refund/revocation flow for any refunded Enrollment)
- **Relationships:**
  - `belongsTo(User::class, 'customer_id')` - customer
  - `belongsTo(Order::class)` - order
...
- **Special Features:** UUID (`uuid7`) generation on create for external references; `ProvisioningPlanResolver` creates the canonical versioned provider matrix; enum-backed `enrollment_status` and `provisioning_status`; date casting for access window; JSON provisioning payloads and plan snapshots. Payment grants the local entitlement (`ACTIVE`) immediately; occupying statuses are `ACTIVE` and `SUSPENDED`, so a paid Customer's seat stays reserved even while provisioning is pending or degraded. Dispatches `EnrollmentStatusChanged` on save/delete to keep projections synchronized. Dispatch is narrowed to occupancy-relevant changes only — the event fires when the enrollment is newly created or when `enrollment_status`/`product_delivery_option_id` change; access-date, notes, and provisioning metadata updates do not dispatch (they do not affect `enrolled_count`/availability). `provisioning_data` carries a per-provider outcome status (`success | failed | manual_action_required | waived`, via `ProvisioningOutcomeStatusEnum`), provider-scoped safe references, a `reconciliation` block (`{status: in_progress|succeeded|failed|manual_action_required|not_applicable}`) written by access-reconciliation attempts, and a top-level `revocation.providers.<provider>` audit block (`{status: revoked, attempt_sequence, data, revoked_at}`) written by the revocation flow for any refunded Enrollment — kept outside `providers.<provider>` so provisioning/reconciliation writes cannot clobber it. Global external enrollment identifiers are removed — identifiers are provider-scoped. Helpers `isRevocationComplete()` / `hasIncompleteRevocation()` expose the revocation lifecycle.
...
- **Special Features:** Attempts use the provider adapter boundary and generic queued job (`ProvisionEnrollmentProviderJob`); attempt and enrollment updates are serialized with row locks, while provider HTTP calls execute outside those locks. Only canonical safe references are written to enrollment provisioning data. Access-reconciliation attempts are identified by `failure_metadata.kind = access_reconciliation` and never degrade the local lifecycle status — they only update `reconciliation.status`.
...
- **`serviceClass(): class-string<AbstractIntegrationService>`** — the integration service that consumes the configuration; the seam the adapter-agreement test iterates. Every case names an adapter — Niliroom included, since its adapter landed — so the test compares each provider's computed state with its `isEnabled()`/`assertConfigured()`/`isReady()` and never skips a provider.
- **`defaultConfig(): array`** — `config/provisioning.php` `providers.<value>` defaults used until the provider is saved; each adapter resolves this same block as its configuration fallback, so it is the single source of a provider's defaults.
