...
- **Special Features:** Moodle attempts use the provider adapter boundary and generic queued job; attempt and enrollment updates are serialized with row locks, while provider HTTP calls execute outside those locks. Only canonical safe references are written to enrollment provisioning data. Access-reconciliation attempts are identified by `failure_metadata.kind = access_reconciliation` and never degrade local enrollment status to `PROVISIONING_FAILED` — they only update `reconciliation.status`.
