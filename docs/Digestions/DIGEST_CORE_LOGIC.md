...
- **RefundBundlePurchaseAction** (`app/Actions/Admin/Refund/RefundBundlePurchaseAction.php`)
  - `handle(BundlePurchase $bundlePurchase, RefundBundlePurchaseData $data): BundleRefundData`: Refunds one Bundle Purchase as a single indivisible operation. Validates that every component item is refundable through `BundlePurchaseRefundGuard` (rejecting already-refunded items and any active component refund), resolves the Bundle-level deduction policy (percentage of the snapshotted Base Value or a fixed amount, rejecting a conflicting pair), distributes it through `BundleRefundDeductionCalculator`, performs exactly one gateway refund for the Bundle total, then completes one component `Refund` per line and writes the policy/effective/redistribution audit into each refund's `transaction_details.bundle_refund`. Component items become `REFUNDED`; their Enrollments become `SUSPENDED` and revocation-pending through `EnrollmentRevocationService`. Unrelated standalone and Bundle units are untouched, and a Digipay Bundle refund inside a mixed order is rejected unless `payments.digipay.allow_partial_refund` is enabled.
- **RefundOrderAction** (`app/Actions/Admin/Refund/RefundOrderAction.php`)
  - `handle(Order $order, RefundOrderData $data): Collection<Refund>`: Refunds a complete mixed Order atomically. It discovers commercial units exactly once — each standalone Order Item individually and each Bundle Purchase as one indivisible unit (internal Bundle Component Order Items are never separate units) — and validates every unit before any gateway call or local financial mutation, so one closed, ineligible, or in-flight-refund unit aborts the whole operation. Percentage deductions are calculated per unit from its snapshotted base value; a fixed deduction is apportioned across units by base-value weight and then distributed within each Bundle by the Bundle rules. Unit base/paid values are derived from the same component snapshots the refund lines use, so the single combined gateway amount always equals the sum of the refund lines. A previously refunded or cancelled standalone unit is skipped; a standalone unit with an in-flight refund aborts the operation; a Bundle Purchase is validated as one indivisible unit. Every refunded unit's Enrollment enters the revocation flow — `SUSPENDED` + revocation-pending, or immediate `REVOKED`/`CANCELLED` when no provider granted external access — and its provider revocations are dispatched after financial success.
- **BundlePurchaseRefundGuard** (`app/Services/BundlePurchaseRefundGuard.php`)
  - `assertRefundable(BundlePurchase): void` and `isFullyHandled(BundlePurchase): bool`: the shared component eligibility rules (empty composition, `REFUNDED`, `CANCELLED`, active refund request) used by both refund actions.
- **BundleRefundDeductionCalculator** (`app/Services/BundleRefundDeductionCalculator.php`)
  - `calculate(int $paidAmount, list<array{...}> $components, int $policyDeductionAmount): array`: Pure, deterministic split of a Bundle deduction. Apportions by snapshotted base price with largest-remainder rounding (stable lowest-index tie-break), caps each share at the component's actual paid allocation, redistributes shortfalls to components with remaining paid value, and caps the effective total at the amount actually paid so a refund can never go negative. Returns per-component policy share, effective deduction, signed redistribution, and refund amount. Uses `WeightedApportionment`.
- **MixedOrderRefundCalculator** (`app/Services/MixedOrderRefundCalculator.php`)
  - `calculate(list<array{...}> $units, ?int $deductionAmount, ?int $deductionPercent): array`: Pure planner for the full-order split. Resolves the policy per unit (percentage from each unit's base value, or a fixed amount apportioned across unit base values with deterministic remainder handling), delegates each unit to `BundleRefundDeductionCalculator` (standalone units are single-component units), and returns per-unit and per-line policy/effective/redistribution/refund amounts plus the totals. A conflicting amount/percent pair throws `deduction_conflict`.
- **WeightedApportionment** (`app/Services/WeightedApportionment.php`)
  - `distribute(int $amount, list<int> $weights): list<int>`: Largest-remainder apportionment with a stable lowest-index tie-break, splitting an integer amount so the parts always sum exactly to it; falls back to an equal split when all weights are zero.
- **RefundPaymentGuard** (`app/Services/RefundPaymentGuard.php`)
  - Shared by both refund actions: `resolveCompletedPayment(Order): ?Payment` and `assertRefundWithinPaymentLimit(Payment, int): void` (cumulative completed refunds plus the new amount must not exceed the payment).
...
- Resolves the sole canonical provider matrix at Enrollment creation. IMS applies when `ims_course_code` is present; the delivery method selects Moodle, SpotPlayer, or Skyroom; a separate numeric `moodle_quiz_course_id` selects Moodle Quiz for non-Moodle delivery methods. `LIVE_SESSION_NILIROOM` selects nothing: Niliroom has no resource to create, so the method plans zero providers (ADR 0013).
...
- **GetEnrollmentDetailAction** (`app/Actions/Shop/Student/GetEnrollmentDetailAction.php`)
  - `handle(User $user, Enrollment $enrollment): EnrollmentDetailData`: Returns enriched enrollment detail — the delivery-option card, `teachers`, `files` (digital assets), `quizzes`, `delivery_access`, `review_info`, `certificate_info` and `survey_block`. Delivery-specific data lives in `delivery_access` (join URL path, course URL, license key, player URL, address, map URL — see the `DeliveryAccessData` entry below); there are no separate per-delivery-method block DTOs.
...
- **ListStudentEnrollmentsAction** (`app/Actions/Shop/Student/ListStudentEnrollmentsAction.php`)
  - `handle(User $user, ProductableEnum $productableType, array $filters, int $perPage): LengthAwarePaginator`: Paginates one customer's enrollments restricted to a single productable type through `Product::ofType()` — the one query shared by the student course and seminar listings, so neither endpoint can leak the other's products (a seminar never appears in the course list and vice versa; digital assets and bundle products appear in neither). Supports `filter[fulfillment_type]` on the delivery option and `filter[name]` on the product, eager-loads `productableWithAllRelations`, `teachers.media` and `orderItem.vendor`, and keeps the query string on the paginator.
...
- **GetTeacherJoinUrlAction** (`app/Actions/Shop/Teacher/GetTeacherJoinUrlAction.php`)
  - `handle(User $user, ProductDeliveryOption $deliveryOption): JoinUrlData`: Generates the teacher's own session login URL for a live seminar, on demand, per request. Gates the delivery method to `DeliveryMethodEnum::getSeminars()` first (`InvalidArgumentException`, `messages.enrollments.not_seminar`). `LIVE_SESSION_SKYROOM` reads `room_id` from the delivery option's `details_json` (a staff-created room — the flow never creates provider rooms) and calls `SkyroomService::createLoginUrl()` with presenter access (`access: 2`, `ttl: 3600`) and username `user-{userId}`, so no Skyroom user is created or duplicated per seminar. The returned `JoinUrlData` carries `type: skyroom` and an `expires_at` derived from the TTL (a Jalali `Verta`, like every response date). The room id follows the same contract as `SkyroomProvisioningProvider` (positive integer, numeric strings accepted); anything else raises `ResourceNotProvisionedException` (`messages.provisioning.skyroom_room_id_missing`), and seminar methods without a handler yet raise `InvalidArgumentException` (`messages.enrollment.delivery_no_join_url`).
  - `LIVE_SESSION_NILIROOM` is served by Niliroom alone — BBB is retired (ADR 0013), so there is no `BbbService` fallback. It reads `nili_room_id` from `details_json` (an opaque public ID staff paste from the panel, so anything but a non-empty string is a delivery-option mistake: `ResourceNotProvisionedException`, `messages.provisioning.niliroom_room_id_missing`), then gates on `NiliroomClientContract::isReady()` — `assertConfigured()` deliberately ignores the `enabled` switch, so the caller owns that check (`messages.enrollments.niliroom_not_configured`). It then delegates the whole three-call flow to `NiliroomService::issueTeacherLoginGrant()` and returns `type: niliroom` with the **provider's own** `expires_at` (a Jalali `Verta`), never a locally computed TTL. Both failures surface as 503 through the controller's existing `ResourceNotProvisionedException` arm.

#### Checkout & Payment Actions (`app/Actions/Shop/*`)
- **CreateOrderFromCartAction** (`app/Actions/Shop/CreateOrderFromCartAction.php`)
  - `handle(CheckoutData $checkoutData, User $user): PaymentProcessResultData`: Wraps the entire checkout pipeline—loads/validates the active cart with `lockForUpdate` (capacity, **registration window, availability window**, publication, Productable-level Purchase Eligibility, order velocity), converts it into `OrderCreateData` inside a DB transaction, and reuses `CreateOrderAction`. `CreateOrderAction` locks the Customer, parent Bundle and component PDO rows in deterministic order, rechecks the Bundle composition version/review/availability/full-payment constraints and Purchase Eligibility, then reserves every actual component atomically. A selected Bundle creates one `BundlePurchase`, internal component Order Items, and AWAITING_PAYMENT component Enrollments; it never creates a parent item or Enrollment. Deletes the cart inside the transaction, then dispatches the selected payment processor **outside** the transaction. Uses `PreparePendingPaymentAction` to create a PENDING Payment before calling `processor->process($payment)`. Free orders (grand total 0) complete immediately through `CompleteFreeOrderPaymentAction` (creates a COMPLETED `NO_PAYMENT` payment record and dispatches `PaymentCompletedEvent` inside the transaction). Returns redirect info for multi-step gateways or finalizes wallet/no-payment flows.
...
- `ACTIVE` and `SUSPENDED` Enrollments block repurchase; only non-occupying states such as `CANCELLED` restore eligibility, keeping incomplete revocation conservative. A `revocation_status` of `pending`, `failed`, or `manual_action_required` blocks eligibility even when the Enrollment is already `CANCELLED`, so a financial refund alone never restores repurchase. The composition setting `products.bundles.allow_repeated_productables` does not weaken ownership or overlap checks.
...
- **Methods:**
  - `issueTeacherLoginGrant(User $user, string $roomId): array{url: string, expires_at: CarbonInterface}`: sync user, enroll as teacher, issue the login grant. `assertConfigured()` runs first, so an unconfigured panel fails before any request.
  - `issueStudentMeetingJoinGrant(User $user, string $roomId): string`: sync user, enroll as student, resolve the room's meeting, then issue the meeting join grant and return its `join_url`. The platform holds no meeting ID: `startOrGetRoomMeeting()` asks the panel for the room's running meeting (the panel starts one when nobody has yet), so all meeting lifecycle logic stays panel-side (ADR 0013) and a student who joins before anyone opened the room still gets a working URL. A meeting join grant carries no lifetime, so there is no expiry.
- **Provider protocol (Niliroom public API v1):** Bearer auth via `withToken()`, `application/json`, base URL = host and `/api/v1` as the path prefix (the IMS convention). Every call is a mutation and carries a freshly generated `Idempotency-Key`; a reused key would be replayed by the provider with its stored response body, which is exactly how a participant would be handed a grant they already redeemed.
  1. `PUT /api/v1/users/eshop/user-{shopUserId}` with `{name, phone}` — one identity per shop user, `provider=eshop`, `subject=user-{id}`; returns the opaque identity public ID that the next calls address. A `data.id` of `null` (the provider's "no identity" case) is unrecoverable.
  2. `PUT /api/v1/rooms/{room}/enrollments/{identity}` with `{role: teacher}` or `{role: student}` — upsert, so an unenrolled participant works on first login. One identity holds either role per room, and the panel derives the meeting role from that enrollment.
  3. Teacher flow: `POST /api/v1/login-grants` with `{user_id, room_id}` — returns `{url, expires_at}`; the single-use redemption URL and its expiry are what the teacher join endpoint hands back (as `type: niliroom`).
  4. Student flow: `POST /api/v1/rooms/{room}/meetings/start` — returns the room's running meeting or starts one, and its `data.id` is the meeting public ID; then `POST /api/v1/meetings/{meeting}/join-grants` with `{user_id}` — addressed to the enrolled identity rather than the `guest` alternative, because a guest grant would mint a throwaway participant outside the room roster; returns `{meeting_id, join_url, guest_id?}`, and the missing `join_url` case is unrecoverable.
- **Error taxonomy:** `handleHttpErrors()` classifies provider responses — every `4xx` (401/403 and the 404/409/422 the provider's own spec declares) is `UnrecoverableProvisioningException`, `5xx` is `RecoverableProvisioningException` — while `ConnectionException` is caught in the shared `send()` wrapper and rethrown as recoverable. A response missing the identity ID, the meeting identity, or the grant URL/expiry is unrecoverable (`messages.integration.niliroom.*`).
- **Tests:** `tests/Integration/Services/Integrations/NiliroomServiceTest.php` fakes the endpoints and asserts both call sequences with payloads and idempotency headers, the returned grant/join URL, fresh keys per call, and the taxonomy.
...
- **Logic:** For each order item with completed status, resolves the canonical plan, then for each planned provider calls `ProvisioningAttemptService::queue()` and dispatches `ProvisionEnrollmentProviderJob`:
  ...
  - `LIVE_SESSION_NILIROOM` delivery → no adapter: Niliroom plans no provider (ADR 0013), so the seminar is active without provisioning
...
### Bundle availability and review
- `BundleAvailabilityService` evaluates the parent Product/Term and every component Product/PDO/Term, date window, review flag, and component capacity. Temporary publication/date/capacity constraints recover automatically when cleared.
- `BundleAvailabilityPropagationService` invalidates containing Bundle Products after component changes and archives affected Bundle PDOs for material reasons (`base_price_changed`, `delivery_method_changed`, `provider_identifier_changed`, `access_duration_changed`, `product_association_changed`, `term_changed`, `component_archived`). A valid Bundle PDO composition edit is NOT material: `UpdateProductDeliveryOptionAction` only increments `composition_version` and invalidates caches so carts require reconfirmation — it never archives or demands review.
- `ReviewBundleAction` is only valid for Bundle PDOs (guard `bundle_review_only_for_bundles`); it revalidates composition, allocations, publication, and eligibility before clearing review state and incrementing `composition_version`. Because review was demanded by hard-archiving the Bundle PDO, a successful review restores its status to `PUBLISHED`, returning the Bundle to sale. Cart items capture the composition version and checkout rejects stale Bundle references.
