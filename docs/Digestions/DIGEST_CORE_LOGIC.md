...
### Bundle Revocation (`app/Services/Provisioning/EnrollmentRevocationService.php`)
- Owns the per-Enrollment external provider revocation lifecycle, independent of provisioning health. `begin()` marks a refunded component Enrollment `SUSPENDED` (still occupying its seat) plus `revocation_status`, and queues one `ProvisioningAttempt` per required provider; supported providers get a `RevokeEnrollmentProviderJob`, unsupported ones become `manual_action_required`. When no provider granted external access it completes immediately (`REVOKED` + `CANCELLED`).
...
- The database allows one active attempt per `(enrollment, provider)` across provisioning, reconciliation, and revocation, so `begin()` waits when a reconciliation is already running. `syncFromReconciliation()` (and the `providerRevoked()` bridge) treat a succeeded cancellation reconciliation as a completed revocation, converging the legacy reconciliation path with the revocation layer. `MoodleProvisioningProvider` and `MoodleQuizProvisioningProvider` implement `RevocationProvider`; IMS, SpotPlayer, BBB, and Skyroom do not.
