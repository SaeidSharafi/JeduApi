...
## Spreadsheet Import/Export Engine (`app/Services/ImportExport/`)

Central engine shared by every importable/exportable resource. Resource names resolve only through the fixed registry; no request value is ever turned into a class name.

- **SpreadsheetResourceRegistry** (`app/Services/ImportExport/SpreadsheetResourceRegistry.php`): fixed map of registered handlers. `import($resource)` returns the registered contract or throws `UnknownImportResourceException` (mapped to 404). `users` is registered; the export handler lands with the User export ticket.
- **Contracts** (`app/Contracts/ImportExport/`): `ImportResourceContract` (`resource()`, `columns()`, `exampleRow()`, `providerCapabilities()`, `validateRow()`). All resource-specific fields, rules, identity decisions and provider flags stay behind this contract — the engine only knows resource names, columns and row results.
- **SpreadsheetColumnDefinition** (`app/Data/ImportExport/SpreadsheetColumnDefinition.php`): one column = machine `key`, localized `headingKey`, locale-independent `aliases` (English + Persian), `required`, `example` and `guidanceKey`. `headings()` accepts the key, the localized heading and every alias, so the generated template always round-trips.
- **ImportPreviewEngine** (`app/Services/ImportExport/ImportPreviewEngine.php`): reads the first worksheet, requires a heading row plus at least one data row, resolves normalized headings, maps cells to column keys, delegates each row to the resource contract, rejects more than 2000 data rows and rejects duplicate identities inside the file by marking every involved row invalid (`code` = `duplicate_identity`). Errors raised here are `ValidationException` messages under the `file` key. The engine deliberately does not inspect worksheets beyond the first, merged cells or cell formatting: the downloadable template defines the shape and the uploader owns it.
- **HeadingNormalizer** (`app/Services/ImportExport/HeadingNormalizer.php`): folds Persian/Arabic letter variants (`ي`/`ى`/`ك`/alef forms), the zero-width non-joiner (kept as a word boundary), zero-width and bidi control characters, diacritics, Persian/Arabic digits and punctuation before comparison.
- **SpreadsheetImportReader** (`app/Services/ImportExport/SpreadsheetImportReader.php`): the maatwebsite/excel 4.x importable itself (`Import`, `ToArray`, `WithLimit`); `read()` returns the raw rows of the first worksheet with a read limit of `MAX_DATA_ROWS + 2`, so an oversized file is rejected without being read fully.
- **ImportTemplateExport** (`app/Services/ImportExport/ImportTemplateExport.php`): builds the official XLSX template from the same column contract used by validation; guidance per column is written as cell comments (heading row) plus one example row, marked by a comment on `A2`. The example row uses an intentionally invalid civil ID so an unedited template cannot silently create a user.
- **UserImportResource** (`app/Services/ImportExport/Resources/UserImportResource.php`): the User vertical slice — 17 columns (required User fields, optional password, additive `provision_moodle`/`provision_ims`/`provision_spotplayer`), create/update rules, enum values accepted as raw values or translated labels, Jalali dates normalized to Gregorian, canonical identity values (phones through `PhoneNumberHelper`, emails lowercased) and phone/email identity matching that never falls back to the other key. Row errors carry stable codes: `required`, `identity_conflict` (`Rule::unique` / `UniqueCivilIdRule`) and `invalid` for everything else. `providerCapabilities()` declares Moodle, IMS and SpotPlayer.
- **CreateImportPreviewAction** (`app/Actions/Admin/ImportExport/CreateImportPreviewAction.php`): validates the upload, stores it privately under `imports/{run_uuid}/`, persists the Import Run and its rows in one transaction (deleting the file if persistence fails) and returns the stable preview envelope with `can_approve` (`preview_ready` with at least one valid row) and the approval warning.
- **ApproveImportRunAction** (`app/Actions/Admin/ImportExport/ApproveImportRunAction.php`): serializes approval with a row lock on the Import Run, requires `include_valid_rows=true` for partial previews, verifies the private upload checksum, revalidates it against the immutable stored snapshot and target resource IDs, then creates/updates every valid local User in one database transaction. Passwords are recovered only from the private upload and remain in memory; invalid rows are skipped. The run moves to `processing` with persisted create/update counts, so repeated approval returns the original result without repeating mutation. Provider dispatch is handled by the later provider-processing phase.
- **BuildImportTemplateAction** (`app/Actions/Admin/ImportExport/BuildImportTemplateAction.php`): streams the generated template as `{resource}-import-template.xlsx`.

Permissions: `imports.preview` (validate an upload), `imports.template` (download a template), and `imports.approve` (atomically commit valid local rows) via `ImportRunPolicy`. Import and export actions are audited by the existing `AdminAuditMiddleware`.
