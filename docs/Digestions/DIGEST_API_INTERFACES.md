...
- Runtime note: a stored `timeout` is only honoured by `ImsService`, which reads `$this->config['timeout']`. `MoodleService` still uses its hardcoded timeout and `SpotPlayerService` reads the config default rather than the stored value, like the SMS areas — making every adapter obey its settings is the deferred runtime seam of spec #98.
