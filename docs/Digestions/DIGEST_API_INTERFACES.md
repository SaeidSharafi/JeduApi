...
- **ProvisioningProviderSettingsController** (`app/Http/Controllers/Api/Admin/Settings/ProvisioningProviderSettingsController.php`):
  ...
  - `show(ProvisioningProviderSettingsEnum $provider)`: **Route:** `GET /api/v1/admin/settings/provisioning-providers/{provider}` - Returns one item shaped exactly like a list element (schema included) for deep links; `bbb`, `moodle_quiz` and any unknown key are `404` via enum route binding.
