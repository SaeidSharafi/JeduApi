...
- `show(Enrollment $enrollment)`: **Route:** `GET /api/v1/shop/student/courses/{enrollment:uuid}` - Returns enriched enrollment detail with typed block DTOs per delivery method, SSO URLs, certificate/review/survey info. **Response DTO:** `EnrollmentDetailData` with nested block DTOs. `review_info` exposes the authenticated student's own review of the enrollment's productable: `has_reviewed` plus `review` (rating, title, comment, translated `status`, `created_at`) for `pending`/`approved` reviews, and `has_reviewed: false` with `review: null` when the student has no live review (including after a `rejected` one).
- **DeliveryAccessData** (shop detail `delivery_access`): typed per-method access block — `type`, computed `is_ready` boolean, `session_label`, `join_url_path`, `course_url`, `completed`, `course_grade`, `license_key`, `player_url`, `address`, `map_url`. `is_ready` is derived from the canonical per-provider outcome status (`success`) — the frontend must render access availability from `is_ready`, never from `enrollment_status` (which is `active` from payment regardless of provisioning state). `live_session_bbb` is the exception: it provisions nothing (ADR 0013), so its `is_ready` is the staff-entered `nili_room_id`'s presence. Provider-specific keys are null unless applicable (`live_session_bbb`/Skyroom → `join_url_path`, the `api.v1.shop.student.courses.join` route path `/api/v1/shop/student/courses/{enrollment:uuid}/join`; Moodle → `course_url`/`completed`/`course_grade`; SpotPlayer → `license_key`/`player_url`; in-person → `address`/`map_url`; direct download → always ready).
