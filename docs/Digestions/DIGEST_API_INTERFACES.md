...
- **ProvisioningProviderSettingsController** (`app/Http/Controllers/Api/Admin/Settings/ProvisioningProviderSettingsController.php`):
  - `index()`: **Route:** `GET /api/v1/admin/settings/provisioning-providers` - Lists every configurable provider from `ProvisioningProviderSettingsEnum` (currently `ims`) as `{key, label, state, schema, settings}`: translated label, grouped `ImsProviderSettingData::schema()` (`general`/`connection`/`credentials`/`advanced`) and effective settings — stored `SettingKeyEnum::IMS` row first, `config/provisioning.php` defaults otherwise, so a never-saved provider is never an empty object. `api_key` is always masked (`***REDACTED***`). No storage key is exposed.
