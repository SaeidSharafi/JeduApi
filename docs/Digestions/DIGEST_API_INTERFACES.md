...
- **ProvisioningProviderSettingsController** (`app/Http/Controllers/Api/Admin/Settings/ProvisioningProviderSettingsController.php`):
  - `index()`: **Route:** `GET /api/v1/admin/settings/provisioning-providers` - Lists every configurable provider from `ProvisioningProviderSettingsEnum` (`ims`, `moodle`, `spotplayer`) in declaration order as `{key, label, state, schema, settings}`: translated label, the provider's grouped schema (`general` for the switch and behaviour defaults, `connection` for the service URL and credentials) and effective settings — the provider's stored setting row first, `config/provisioning.php` defaults otherwise, so a never-saved provider is never an empty object. Sensitive fields (`api_key`, `token`, `auth_userkey_token`) are always masked (`***REDACTED***`). No storage key is exposed.
