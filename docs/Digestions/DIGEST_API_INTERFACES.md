...
### SMS Notification Settings Controller
- **SmsNotificationSettingsController** (`app/Http/Controllers/Api/Admin/Settings/SmsNotificationSettingsController.php`):
  - `index()`: **Route:** `GET /api/v1/admin/settings/sms-notifications` - Returns the shared `SmsNotificationOptionData::schema()` (`general`: `enabled`, `pattern_code`) plus all five `SmsNotificationOptionEnum` options in declaration order (`otp`, `refund_completed`, `order_paid`, `enrollment_ready`, `wallet_campaign_credited`) as `{key, label, state, settings}`. Each option's settings merge the stored `SettingKeyEnum::SMS_NOTIFICATIONS` entry field-by-field over its `config/sms.php` default, so a never-saved option still returns both fields; `pattern_code` is `""` (never `null`) when unset.
  - `update(UpdateSmsNotificationsData $request)`: **Route:** `PUT /api/v1/admin/settings/sms-notifications` - Body carries an `options` map keyed by option key; **Delegates to:** `UpdateSmsNotificationsAction`. An omitted option is left untouched, a provided option must carry both `enabled` and `pattern_code`, and an unknown option key is a `422`. Every handler authorises with `Gate::authorize('viewAny'|'update', Setting::class)`.
- Each option's read `state` is `{enabled, configured, ready}`: `configured` is true when the option requires no pattern or its `pattern_code` is non-empty, and `ready` is `enabled && configured` — so an enabled option without its pattern code is visibly not ready.

### Provisioning Provider Settings Controller
- **ProvisioningProviderSettingsController** (`app/Http/Controllers/Api/Admin/Settings/ProvisioningProviderSettingsController.php`):
  - `index()`: **Route:** `GET /api/v1/admin/settings/provisioning-providers` - Lists every configurable provider from `ProvisioningProviderSettingsEnum` (`ims`, `moodle`, `spotplayer`, `skyroom`, `niliroom`) in declaration order as `{key, label, state, schema, settings}`: translated label, the provider's grouped schema (`general` for the switch and behaviour defaults, `connection` for the service URL and credentials) and effective settings — the provider's stored setting row first, `config/provisioning.php` defaults otherwise, so a never-saved provider is never an empty object. Sensitive fields (`api_key`, `token`, `auth_userkey_token`, `api_token`) are always masked (`***REDACTED***`). Skyroom's `base_url` is optional and falls back to the shipped Skyroom API endpoint; Niliroom's credential field is `api_token`. No storage key is exposed.
  - `show(ProvisioningProviderSettingsEnum $provider)`: **Route:** `GET /api/v1/admin/settings/provisioning-providers/{provider}` - Returns one item shaped exactly like a list element (schema included) for deep links; `moodle_quiz` and any unknown key are `404` via enum route binding.
  - `update(Request $request, ProvisioningProviderSettingsEnum $provider)`: **Route:** `PUT /api/v1/admin/settings/provisioning-providers/{provider}` - Flat body using the same keys as `schema`. The controller resolves the provider's own data class from the enum and validates it against that class's rules (the body is flat, so no single bound DTO fits every provider). **Delegates to:** `UpdateProvisioningProviderSettingAction`, which applies full-replace semantics to non-secret fields, keeps each stored secret when it is omitted/`null`/`***REDACTED***`, clears it on `""`, and rejects enabling a provider whose schema is not configured with a `422` naming every offending field. Moodle carries two separate credentials — the service token (`token`) for administrative calls and the login token (`auth_userkey_token`) for `auth_userkey_request_login_url` — and both are required before it is configured (ADR 0011). Every handler authorises with `Gate::authorize('viewAny'|'update', Setting::class)`.
- Provider `state` is `{enabled, configured, ready}`: `configured` is derived from the provider's schema (every required non-boolean field filled), never from the adapter, and `ready` is `enabled && configured` — a disabled-but-complete provider is configured, an enabled-but-incomplete one is not ready. `tests/Integration/Services/Provisioning/ProvisioningProviderSettingsAgreementTest.php` asserts the computed values equal the adapter's own `isEnabled()`/`assertConfigured()`/`isReady()` for complete, incomplete and disabled configurations; every case maps to an adapter, Niliroom's `NiliroomService` included, so no provider is skipped (ADR 0013 keeps Niliroom as the only panel behind BBB live sessions).
- Runtime note: each provider adapter resolves `config/provisioning.php` `providers.<key>` as its configuration fallback — the same block the panel reads — while a stored setting row still wins. A stored `timeout` is honoured by `ImsService` and `SpotPlayerService`; `MoodleService` still uses its hardcoded timeout, which remains part of the deferred runtime seam of spec #98.
...
- **ProvisioningSummaryData** (nested in every admin enrollment response): `status` (aggregate `ProvisioningStatusEnum`: ready/in_progress/healthy/degraded/manual_action_required), `plan` (`ProvisioningPlanData`: `version`, `providers[]` each with `provider`, `applicable`, `readiness` (`ProvisioningReadinessEnum`: ready/disabled/invalid), `configuration_issue`; plus `status`, `resolved_at`), and `reconciliation_status` (null or in_progress/succeeded/failed/manual_action_required/not_applicable).
- **ChangeEnrollmentStatusController** (`app/Http/Controllers/Api/Admin/Enrollment/ChangeEnrollmentStatusController.php`):
  - `__invoke(Enrollment $enrollment, EnrollmentStatusChangeData $data, ChangeEnrollmentStatusAction $action)`: **Route:** `POST /api/v1/admin/enrollments/{enrollment}/change-status` - **Request DTO:** EnrollmentStatusChangeData (new_status, reason) - **Response DTO:** EnrollmentData - Valid transitions: `awaiting_payment → cancelled`, `active → suspended|expired|cancelled`, `suspended → active|cancelled`. Every transition records access-reconciliation attempts; providers without reconciliation support become manual-action-required.
- **RetryProvisioningController** (`app/Http/Controllers/Api/Admin/Enrollment/RetryProvisioningController.php`):
  - `__invoke(Enrollment $enrollment, RetryProvisioningAction $action)`: **Route:** `POST /api/v1/admin/enrollments/{enrollment}/retry-provisioning` - **Authorization:** `Gate::authorize('retryProvisioning', $enrollment)` via `PermissionEnum::ENROLLMENT_RETRY_PROVISION` - **Response:** `{message, providers[]}` - Delegates to RetryProvisioningAction
  - The same controller accepts an optional provider route segment at `POST /api/v1/admin/enrollments/{enrollment}/retry-provisioning/{provider}` for provider-specific retries; duplicate active attempts are deduplicated transactionally and at the queue boundary. Retry eligibility is owned by provisioning state (failed/manual-action providers, or never-attempted enrollments), not the lifecycle status.
- **ResolveProvisioningController** (`app/Http/Controllers/Api/Admin/Enrollment/ResolveProvisioningController.php`):
  - `__invoke(ManualProvisioningResolutionData $data, Enrollment $enrollment, ManualProvisioningRecoveryAction $action)`: **Route:** `POST /api/v1/admin/enrollments/{enrollment}/provisioning/resolve` - **Authorization:** `resolveProvisioning` - **Request DTO:** ManualProvisioningResolutionData (provider, provider-specific safe references, reason) - **Response DTO:** EnrollmentData - Records a staff-attributed manual resolution attempt.
- **WaiveProvisioningController** (`app/Http/Controllers/Api/Admin/Enrollment/WaiveProvisioningController.php`):
  - `__invoke(ManualProvisioningWaiverData $data, Enrollment $enrollment, ManualProvisioningRecoveryAction $action)`: **Route:** `POST /api/v1/admin/enrollments/{enrollment}/provisioning/waive` - **Authorization:** `waiveProvisioning` via `PermissionEnum::ENROLLMENT_WAIVE_PROVISION` - **Request DTO:** ManualProvisioningWaiverData (provider, reason) - **Response DTO:** EnrollmentData - Exceptional waiver with mandatory reason.
- **ProvisioningDiagnosticsController** (`app/Http/Controllers/Api/Admin/Enrollment/ProvisioningDiagnosticsController.php`):
  - `__invoke(Enrollment $enrollment, ProvisioningDiagnosticsService $diagnostics)`: **Route:** `GET /api/v1/admin/enrollments/{enrollment}/provisioning/diagnostics` - **Authorization:** `viewProvisioningDiagnostics` - **Response DTO:** `ProvisioningDiagnosticsData` — `status` (aggregate), `providers[]` (`ProvisioningDiagnosticData`: `provider`, `status` (pending/succeeded/failed/manual_action_required/waived), `retryable`, `recommended_action`, `safe_error` (sanitized validation field/message details when available; generic for other failures), `references` (allow-listed safe refs only), `updated_at`), `reconciliation_status`.
- **AdvancedProvisioningDiagnosticsController** (`app/Http/Controllers/Api/Admin/Enrollment/AdvancedProvisioningDiagnosticsController.php`):
  - `__invoke(Enrollment $enrollment, ProvisioningDiagnosticsService $diagnostics)`: **Route:** `GET /api/v1/admin/enrollments/{enrollment}/provisioning/diagnostics/advanced` - **Authorization:** `viewDiagnostics` - **Response DTO:** `AdvancedProvisioningDiagnosticsData` wrapping the summary plus chronological attempt history (`ProvisioningAttemptDiagnosticData`: provider, status, retryable, sequence, trigger, failure_code, classification, correlation_id, safe context including sanitized `validation_errors` when available, timestamps). Secrets and raw provider payloads are never returned.
- **ProvisioningPlanController** (`app/Http/Controllers/Api/Admin/Enrollment/ProvisioningPlanController.php`):
  - `preview(Enrollment $enrollment, ManualProvisioningRecoveryAction $action)`: **Route:** `GET /api/v1/admin/enrollments/{enrollment}/provisioning-plan/preview` - **Authorization:** `resolveProvisioning` - **Response DTO:** `ProvisioningPlanPreviewData` (current_version, next_version, added[], removed[], changed[]) — explicit provider diff before rebuilding.
  - `apply(ProvisioningPlanApplyData $data, Enrollment $enrollment, ManualProvisioningRecoveryAction $action)`: **Route:** `POST /api/v1/admin/enrollments/{enrollment}/provisioning-plan/apply` - **Authorization:** `resolveProvisioning` - **Request DTO:** ProvisioningPlanApplyData (confirm) - **Response DTO:** EnrollmentData - Rebuilds the plan, increments version, preserves prior snapshot in `plan_history`.
...
##### CourseController (`app/Http/Controllers/Api/Shop/Student/CourseController.php`)
- `index()`: **Route:** `GET /api/v1/shop/student/courses` - Lists the authenticated user's enrolled course delivery options. Scoped to `products.productable_type = course`; seminars and digital assets are excluded (they have their own listings), and a Bundle never carries its own enrollment. Shares its query, filters (`filter[fulfillment_type]`, `filter[name]`, `per_page`) and pagination with the seminar listing through `ListStudentEnrollmentsAction`. **Response DTO:** Paginated collection of enrollment list DTOs with delivery method, dates, provisioning status.
- `show(Enrollment $enrollment)`: **Route:** `GET /api/v1/shop/student/courses/{enrollment:uuid}` - Returns enriched enrollment detail with typed block DTOs per delivery method, SSO URLs, certificate/review/survey info. Returns 404 (`messages.enrollments.not_found`) when the enrollment belongs to another customer **or** its productable is not a course; the two cases are indistinguishable to the caller. **Response DTO:** `EnrollmentDetailData` with nested block DTOs. `review_info` exposes the authenticated student's own review of the enrollment's productable: `has_reviewed` plus `review` (rating, title, comment, translated `status`, `created_at`) for `pending`/`approved` reviews, and `has_reviewed: false` with `review: null` when the student has no live review (including after a `rejected` one).
- **DeliveryAccessData** (shop detail `delivery_access`): typed per-method access block — `type`, computed `is_ready` boolean, `session_label`, `join_url_path`, `course_url`, `completed`, `course_grade`, `license_key`, `player_url`, `address`, `map_url`. `is_ready` is derived from the canonical per-provider outcome status (`success`) — the frontend must render access availability from `is_ready`, never from `enrollment_status` (which is `active` from payment regardless of provisioning state). `live_session_niliroom` is the exception: it provisions nothing (ADR 0013), so its `is_ready` is the staff-entered `nili_room_id`'s presence. Provider-specific keys are null unless applicable (`live_session_niliroom`/Skyroom → `join_url_path`, the `api.v1.shop.student.seminars.join` route path `/api/v1/shop/student/seminars/{enrollment:uuid}/join`; Moodle → `course_url`/`completed`/`course_grade`; SpotPlayer → `license_key`/`player_url`; in-person → `address`/`map_url`; direct download → always ready).

##### SeminarController (`app/Http/Controllers/Api/Shop/Student/SeminarController.php`)
- `index()`: **Route:** `GET /api/v1/shop/student/seminars` - Lists the authenticated user's enrolled seminar delivery options, scoped to `products.productable_type = seminar`. Reuses the course listing's query, filters and pagination through `ListStudentEnrollmentsAction`. **Response DTO:** the same paginated enrollment list DTOs as `CourseController::index()`.
- `show(Enrollment $enrollment)`: **Route:** `GET /api/v1/shop/student/seminars/{enrollment:uuid}` - Returns the identical `EnrollmentDetailData` contract as `CourseController::show()`, with the productable restricted to seminars and the same indistinguishable 404 for a non-owner or wrong-type enrollment. Does not trigger the Moodle progress sync, which is course-only.
...
- `__invoke()`: **Route:** `GET /api/v1/shop/student/quizzes` - Returns list of user's quizzes with completion states, sourced from Moodle integration. Each quiz activity's `cid` is Moodle's course-module ID (`cmid`); pass that value as `{courseModuleId}` to `POST /api/v1/shop/student/quizzes/{courseModuleId}/moodle/sso`. Cached per user through the `CacheStore` gateway under `CacheKey::StudentQuizzes` (`CacheTag::Content`); the key is forgotten by Moodle progress sync and by provisioning/revocation for that user.

##### StudentQuizMoodleSsoController (`app/Http/Controllers/Api/Shop/Student/StudentQuizMoodleSsoController.php`)
- `__invoke(int $courseModuleId)`: **URL Param:** `courseModuleId` is the Moodle course-module ID (`cmid`) from the quiz activity's `cid` field in the quiz list response (for example, `cid: 841`). **Route:** `POST /api/v1/shop/student/quizzes/{courseModuleId}/moodle/sso` - Checks the authenticated student's current Moodle course and visible quiz access without relying on the cached list or a Shop enrollment. Returns 404 when access is absent. Derives `/mod/quiz/view.php?id={courseModuleId}` on the server and ignores client `wantsurl`. **Response DTO:** `MoodleSsoUrlData`.
...
##### QuizController (`app/Http/Controllers/Api/Shop/Teacher/QuizController.php`)
- `__invoke()`: **Route:** `GET /api/v1/shop/teacher/quizzes` - Returns Moodle quizzes for courses where the authenticated teacher has teaching capability. Each quiz activity's `cid` is Moodle's course-module ID (`cmid`); pass that value as `{courseModuleId}` to `POST /api/v1/shop/teacher/quizzes/{courseModuleId}/moodle/sso`.

##### TeacherQuizMoodleSsoController (`app/Http/Controllers/Api/Shop/Teacher/TeacherQuizMoodleSsoController.php`)
- `__invoke(int $courseModuleId)`: **URL Param:** `courseModuleId` is the Moodle course-module ID (`cmid`) from the quiz activity's `cid` field in the quiz list response (for example, `cid: 841`). **Route:** `POST /api/v1/shop/teacher/quizzes/{courseModuleId}/moodle/sso` - Requires a linked Teacher profile and current Moodle course teaching capability for the quiz. Does not require a Shop delivery option. Returns 404 when Moodle access is absent. Derives the quiz destination on the server and ignores client `wantsurl`. **Response DTO:** `MoodleSsoUrlData`.
...
- **CourseController::show()** (`GET /api/v1/shop/student/courses/{enrollment:uuid}`): Triggers `SyncMoodleProgressJob` after rendering response to sync Moodle course progress (throttled at 5-min per enrollment). The sync result is stored in `provisioning_data` for subsequent reads. Seminar enrollments served by `SeminarController::show()` do not trigger this sync.
