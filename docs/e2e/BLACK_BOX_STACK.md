...
Moodle/Moodle Quiz, SpotPlayer, BBB, and Skyroom. The Niliroom live-session
client (`NiliroomClientContract`) is simulated too, but it is not a provisioning
provider, so it is asserted explicitly instead of through the provider loop.
