...
<text x="50%" y="90%" text-anchor="middle" fill="#687787" font-size="16">{$tag}</text>
</svg>
SVG;
                $name = "fake-{$tag}-".uniqid();
                $media = MediaUploader::fromString($svg)
                    ->useFilename($name)
                    ->toDirectory('fake-media/uploads')
                    ->withAltAttribute("alt-{$name}")
                    ->upload();
                $course->attachMedia($media, $tag);
            }
        });
