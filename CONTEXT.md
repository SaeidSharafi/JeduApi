...
An external system that a customer's or teacher's access is delivered through: IMS, Moodle, SpotPlayer, and the live-session panels Skyroom and Niliroom. `live_session_niliroom` is a delivery method, not a provider: it is served by Niliroom alone, and the legacy BigBlueButton integration is retired (ADR 0013). Moodle Quiz has no credentials of its own: it runs on the Moodle configuration.
_Avoid_: enrollment provider, integration, gateway, BBB as a provider
